    ...
    IERC721Receiver
{
    ...
    constructor(
        ...
        address admin,
        address whitelister
    ) EIP712Whitelisting(whitelister, "PartyBasket") {
        require(
            _lootboxAddress != address(0),
            "Lootbox address cannot be the zero address"
        );
        require(
            admin != address(0),
    ...
    function redeemBounty(bytes calldata signature, uint256 nonce)
        public
        ...
        whenNotPaused
    {
        ...
        LootboxEscrow(payable(lootboxAddress)).transferFrom(
            address(this),
            msg.sender,
            nextAvailableNFT
        );
    ...
    function viewNFTs() public view returns (uint256[] memory) {
        uint256[] memory listOfNFTsAvailable = LootboxEscrow(
            payable(lootboxAddress)
        ).viewAllTicketsOfHolder(address(this));
