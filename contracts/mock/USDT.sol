...
contract USDT is LibNote {
  ...
  event Transfer(address indexed src, address indexed dst, uint wad);
    ...
    // --- ERC20 Data ---
    string  public constant name     = "Tether USD Stablecoin";
    ...
    function mint(address usr, uint wad) external {
      ...
      if(wards[msg.sender] == 0) {
        require(add(wad, daiMintedToday[msg.sender]) <= dailyDAILimit || sub(block.number, lastMintRestart[msg.sender]) >= 6500 && wad <= dailyDAILimit, "Over daily USDT Limit");
